...
async function freshDeploy() {
  ...
  const changer = await ChangerFactory.deploy(inrate.target, moc.target, bitProRate, commissions);

  return { inrate, moc, changer, bitProRate, commissions };
}
...
describe('RemovePanicButtonProposal', function () {
  ...
  it('execute() applies, makes MoC unstoppable and burns fuse', async () => {
    const { changer, inrate, moc, bitProRate, commissions } = await freshDeploy();
