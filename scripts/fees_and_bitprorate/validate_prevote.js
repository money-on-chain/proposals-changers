...
async function main() {
  ...
  if (v1List.length === Object.keys(MAP_TX_V1).length) {
    ...
    console.log(`ERROR. commissionRates length = ${v1List.length}, expected ${Object.keys(MAP_TX_V1).length}`);
  }
  ...
  for (let i = 0; i < v1List.length; i++) {
    ...
    console.log(expFee !== undefined
      ? `OK.  V1[${i}] ${knownName} txType=${txType}`
      : `ERROR. V1[${i}] txType=${txType} is not expected by current MAP_TX_V1`
    );

    if (expFee !== undefined) {
      console.log(feeBN === expFee
        ? `OK.  V1[${i}] ${knownName} fee (raw=${feeBN.toString()} ~ ${pretty(feeBN)})`
        : `ERROR. V1[${i}] ${knownName} fee mismatch (on=${feeBN.toString()} ~ ${pretty(feeBN)}, exp=${expFee.toString()} ~ ${pretty(expFee)})`
      );
    }
  }
  ...
  if (v1List.length > Object.keys(MAP_TX_V1).length) {
    console.log(`WARN. commissions has ${v1List.length - Object.keys(MAP_TX_V1).length} extra entries beyond expected`);
  }
  console.log();
  ...
  for (let i = 0; i < v2List.length; i++) {
    ...
    console.log(expVal !== undefined
      ? `OK.  ROCV2[${i}] ${friendlyName} present`
      : `WARN. ROCV2[${i}] ${friendlyName} not found in config (might be extra on-chain entry)`
    );

    if (expVal !== undefined) {
      console.log(valueBN === expVal
        ? `OK.  ROCV2[${i}] ${friendlyName} value (raw=${valueBN.toString()} ~ ${pretty(valueBN)})`
        : `ERROR. ROCV2[${i}] ${friendlyName} mismatch (on=${valueBN.toString()} ~ ${pretty(valueBN)}, exp=${expVal.toString()} ~ ${pretty(expVal)})`
      );
    }
  }
