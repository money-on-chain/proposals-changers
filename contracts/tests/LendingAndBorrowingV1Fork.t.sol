...
interface IBufferCoinbaseLike {
    function outputs(uint256 idx) external view returns (address);
...
interface IMocSwapperV3MultiHopProbe {
    ...
    function maxAmountToSwapProviders(address tokenIn, address tokenOut) external view returns (address);
}
...
contract LendingAndBorrowingV1ForkTest is Test {
    ...
    function setUp() public {
        ...
        mocSwapperCoreV1 = address(new MocSwapperCoreV1(governor, mocV1, mocStateV1, mocInrateV1, docToken));
        ...
        address priceProviderInverse;
        {
            ...
            assembly { priceProviderInverse := create(0, add(code, 0x20), mload(code)) }
        }
        ...
        address bufferImpl;
        {
            ...
            assembly { bufferImpl := create(0, add(bufferCode, 0x20), mload(bufferCode)) }
        }
    ...
    function testFork_BufferCoinbase_OutputsConfiguredCorrectly() public view {
        ...
        assertEq(
            buffer.outputs(1),
            bufferOutput1,
            "BufferCoinbase outputs[1] should be bufferOutput1"
        );
        assertEq(
            buffer.outputs(2),
            bufferOutput2,
            "BufferCoinbase outputs[2] should be bufferOutput2"
        );
    }
    ...
    function testFork_EndToEnd_BitProInterestFlow() public {
        ...
        assertGt(bufferBalanceAfter, bufferBalanceBefore, "Buffer should have received RBTC from interest payment");
        ...
        assertGt(output0Balance, 0, "output[0] (reverseAuction) internal balance should be > 0 after liquidate");
        assertGt(output1Balance, 0, "output[1] (bufferOutput1) internal balance should be > 0 after liquidate");
        assertEq(output0Balance + output1Balance, rbtcReceived, "Sum of output balances should equal RBTC received");
        ...
        assertGt(docAfter, docBefore, "outputAccount (tpInjector) should have received DOC after triggerOrders()");
    }
    ...
    function _readParamsFromJson() internal {
        ...
        docToRbtcPriceProvider = vm.parseJsonAddress(
            json,
            _key(module, "docToRbtcPriceProvider")
        );
